...
void CoinDetector::find_circles(cv::Mat image, cv::Mat &output_image){
	...
	cv::HoughCircles(image, coin_positions, CV_HOUGH_GRADIENT, 2.0, 40, 200, 130, 14, 83);
	cv::vector<int> toremove;
	//cv::HoughCircles(image, coin_positions, CV_HOUGH_GRADIENT, 2.0, 40, 200, 130, min_circle_radius, max_circle_radius);
	/*for (int i = 0; i < coin_positions.size()-1; i++) {
		for (int j = i+1; j < coin_positions.size(); j++){
			int d1 = sqrt((coin_positions[i][0] - coin_positions[j][0]) * 2 + (coin_positions[i][1] - coin_positions[j][1]) * 2);
			int r12 = coin_positions[i][2] + coin_positions[j][2];
			if ((d1 - r12) < -3)
			{
	...
	if (toremove.size() > 0)
	{
		for (int k = 0; k < toremove.size(); k++){
			coin_positions.erase(toremove[k]);
		}
	}*/
...
void CoinDetector::isolate_coins(cv::Mat image, cv::vector<cv::Mat> &output_coin_images) {
	...
	//std::cout << "isolate coins" << std::endl;
	...
	for (int i = 0; i < coin_positions.size(); i++) {
		...
		//std::cout << "for loop image" << std::endl;
		int x = (coin_positions[i][0] - coin_positions[i][2] * ratio)<0 ? 1 : (coin_positions[i][0] - coin_positions[i][2] * ratio);
		int y = (coin_positions[i][1] - coin_positions[i][2] * ratio)<0 ? 1 : (coin_positions[i][1] - coin_positions[i][2] * ratio);
		...
		int width = (x + 2 * w_max)<image.cols ? (2 * w_max) : image.cols - x;
		int height = (y + 2 * h_max)<image.rows ? (2 * h_max) : image.rows - y;
